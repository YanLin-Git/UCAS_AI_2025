...
<details>
<summary>在<b>MHVAE</b>基础上，添加约束</summary>

![VDM](pngs/VDM.png)
> 1. 潜在空间的维度与数据的维度完全相同
> 2. 每个时间步的`encoder输出`不再用模型来拟合，而是预定义好的高斯分布
>    - $q(x_t|x_{t-1})$不需要训练，是一个确定的`线性高斯变换`
>    - 只训练`decoder`

</details>

<details>
<summary>怎么来理解呢？</summary>
...
类似于**VAE**，**VDM**也可以推导出**ELBO**，并分解：
![VDM_ELBO](pngs/VDM_ELBO.png)

## 4.3 DDPM（Denoising Diffusion Probabilistic Models）

**VDM**的一种具体实现。  
经过一系列数学推导，可将**VDM**中，分解后的**ELBO**进一步简化为：
$$
    \argmin\limits_\theta ||\epsilon_0 - \hat{\epsilon}_\theta(x_t, t) ||_2^2
$$
> 1. $\epsilon_0$ 为从 $x_0$ 到 $x_t$ ，添加的真实噪声
> 2. $\hat{\epsilon}_\theta(x_t, t)$ 为给定输入 $x_t、t$ 后，模型预测的噪声

## 4.4 DDIM（Denoising Diffusion Implicit Models）

**DDPM**中，为了从 $x_T$ 还原 $x_0$，我们需要一步步降噪：
$$
    x_T \to \dots \to x_{t+1} \to x_t \to x_{t-1} \to \dots \to x_0
$$

**DDIM**做了简化，例如从 $x_{100}$ 还原到 $x_0$，可以跨步来生成：
$$
    x_{100} \to x_{80} \to x_{60} \to x_{40} \to x_{20} \to x_{0}
$$
